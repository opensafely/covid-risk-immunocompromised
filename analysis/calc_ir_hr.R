...
if (subgroup=="Tx") {
  subgroups_vctr = subgroups_vctr[!subgroups_vctr %in% c("any_bone_marrow_type", "any_bone_marrow_cat", "radio_chemo_cat", "immunosuppression_medication_cat", "immunosuppression_diagnosis_cat", 
                                                         "any_transplant", "ckd_rrt")] #ckd_rrt removed due to colinearity with kidney transplant
}
if (subgroup=="HC") {
  subgroups_vctr = subgroups_vctr[!subgroups_vctr %in% c("any_transplant_type", "any_transplant_cat", "radio_chemo_cat", "immunosuppression_medication_cat", "immunosuppression_diagnosis_cat", 
                                                         "any_transplant", "any_bone_marrow", "immunosuppression_diagnosis")] # immunosuppression diagnosis removed due to colinearity with haem cancer
}
if (subgroup=="RC") {
...
for (o in 1:length(outcomes)) {
  ...
  for (s in 1:length(subgroups_vctr)) {
    ...
    model_cols = c("reference_row_min", 
                   "n_obs_min", "n_event_min", "exposure_min", "estimate_min", "std.error_min", "statistic_min", 
    ...
    # If eligible, proceed with minimal and adjusted models
      ...
      if (group == "agegroup") {
        cox_minimal = coxph(as.formula(paste0("Surv(follow_up, ind) ~ factor(agegroup) + sex + strata(region)")), 
                            data = data_filtered)
      } else if (group == "agegroup_broad") {
        cox_minimal = coxph(as.formula(paste0("Surv(follow_up, ind) ~ factor(agegroup_broad) + sex + strata(region)")), 
                            data = data_filtered)
      } else if (group == "sex") {
      ...
      if (wave=="wave3" | wave=="wave4" | wave=="wavejn1") {
        if (group == "agegroup") {
          ...
          cox_adj = coxph(as.formula(paste0("Surv(follow_up, ind) ~ factor(agegroup_broad) + sex + pre_wave_vaccine_group + pre_wave_infection_group + strata(region)")), 
                          data = data_filtered)
        ...
        if (group == "agegroup") {
          cox_adj = coxph(as.formula(paste0("Surv(follow_up, ind) ~ factor(agegroup) + sex + pre_wave_infection_group + strata(region)")), 
                          data = data_filtered)
          
        } else if (group == "agegroup_broad") {
          cox_adj = coxph(as.formula(paste0("Surv(follow_up, ind) ~ factor(agegroup_broad) + sex + pre_wave_infection_group + strata(region)")), 
                          data = data_filtered)
          
        } else if (group == "sex") {
          cox_adj = coxph(as.formula(paste0("Surv(follow_up, ind) ~ rcs(age, 4) + factor(sex) + pre_wave_infection_group + strata(region)")), 
                          data = data_filtered)
      ...
      if (wave!="wave1") {
        ...
        ir_crude[,c("n_obs_adj", "n_event_adj", "exposure_adj", "estimate_adj", "std.error_adj", "statistic_adj", 
                   "p.value_adj", "conf.low_adj", "conf.high_adj")] = NA
      }
      ...
      if (wave=="wave3" | wave=="wave4" | wave=="wavejn1") {
        ...
        if (group == "agegroup") {
          ...
          cox_adj = coxph(as.formula(paste0("Surv(follow_up, ind) ~ factor(agegroup_broad) + sex + pre_wave_vaccine_group + pre_wave_infection_group +
                                ethnicity + imd + multimorb_cat + strata(region)")), 
                          data = data_filtered)
        ...
        # exclude multimorb from contributing morbidities due to colinearity
        } else if (group %in% c("chronic_respiratory_disease", "asthma", "chronic_cardiac_disease", "bp_ht",
                                "ckd_rrt", "chronic_liver_disease", "stroke", "dementia", "other_neuro", 
                                "learning_disability", "diabetes_controlled", "asplenia", "bmi", "sev_mental_ill")) {
        ...
        if (group == "agegroup") {
          cox_adj = coxph(as.formula(paste0("Surv(follow_up, ind) ~ factor(agegroup) + sex + pre_wave_infection_group +
                                ethnicity + imd + multimorb_cat + strata(region)")), 
                          data = data_filtered)
          
        } else if (group == "agegroup_broad") {
          cox_adj = coxph(as.formula(paste0("Surv(follow_up, ind) ~ factor(agegroup_broad) + sex + pre_wave_infection_group +
                                ethnicity + imd + multimorb_cat + strata(region)")), 
                          data = data_filtered)
          
        } else if (group == "sex") {
          cox_adj = coxph(as.formula(paste0("Surv(follow_up, ind) ~ rcs(age, 4) + factor(sex) + pre_wave_infection_group + 
                                          ethnicity + imd + multimorb_cat + strata(region)")), 
                          data = data_filtered)
        ...
        if (group == "agegroup") {
          cox_adj = coxph(as.formula(paste0("Surv(follow_up, ind) ~ factor(agegroup) + sex +
                                ethnicity + imd + multimorb_cat + strata(region)")), 
                          data = data_filtered)
          
        } else if (group == "agegroup_broad") {
          cox_adj = coxph(as.formula(paste0("Surv(follow_up, ind) ~ factor(agegroup_broad) + sex +
                                ethnicity + imd + multimorb_cat + strata(region)")), 
                          data = data_filtered)
          
        } else if (group == "sex") {
          cox_adj = coxph(as.formula(paste0("Surv(follow_up, ind) ~ rcs(age, 4) + factor(sex) + 
                                          ethnicity + imd + multimorb_cat + strata(region)")), 
                          data = data_filtered)
    ...
    # Secondary redactions - retain outputs if at least 2 groups with sufficient event counts
